...
--Q. Update an Existing Member's Address
UPDATE members
SET member_address = '125 Oak St'
WHERE member_id = 'C103';

--Q. Delete a Record from the Issued Status Table
DELETE FROM issued_status
WHERE  issued_id =   'IS121';


select count(issued_emp_id) as count_emp, issued_emp_id 
from issued_status as i 
left join members as m 
on i.issued_emp_id = m.member_id 
group by i.issued_emp_id 
having count(issued_emp_id) > 1;	

--Q. List Members Who Registered in the Last 180 Days
select member_id from members
where reg_date >= current_date - interval '180 days';

--Q. Retrieve All Books in a Specific Category
SELECT * FROM books WHERE category = 'Classic';

--Q. Create a summary table of books and no: of times issued
...
-- Q. find no: of books in each category
select count(isbn), category from books
group by category;

-- Q. Find Total Rental Income by book
select 
issued_book_name,
issued_book_isbn,
rental_price,
...
-- Q. Find Total Rental Income by Category
select 
category,
count(issued_id) as no_of_times_issued,
sum(rental_price)
...
-- Q. List Employees with Their Branch Manager's Name and their branch details
...
-- Q. Create a Table of Books with Rental Price Above a Certain Threshold
...
-- Q. Retrieve the List of Books Not Yet Returned
...
-- Q. Update Book Status on Return
...
--Q. CTAS: Create a Table of active_members containing members who have issued at least one book in the last 2 months.
...
--Q. Write a query to find the top 3 employees who have processed the most book issues. 
...
--Q: Stored Procedure Objective: Create a stored procedure to manage the status of books in a library system. 
...
--Q. Create Table As Select (CTAS) Objective: Create a CTAS (Create Table As Select) query to identify overdue
--books and calculate fines. Description: Write a CTAS query to create a new table that lists each member 
...
--The number of books issued by each member. The resulting table should show: Member ID Number of overdue books Total 
--fines
